...
| `MicroLesson.contentJson` (already-created rows, all users) | **Leave as legacy** | None — historical rows are not rewritten | Existing rows simply have no `sourceRuleCodes` (new field, nullable/empty-array default) | N/A | `MicroLessonsService.getById()` continues to work unmodified for pre-existing rows |
