...
- **sourceRefs:** `CATEGORY_SIMPLIFIED_RULE.ARTICLES` (the portion); `MICRO_LESSON_GENERIC_EXERCISES.ARTICLES.fillBlank`; British Council LearnEnglish — "The definite article: 'the'" and "Articles: 'a', 'an', 'the'" (`grammar-source-verification.md` #2) — confirms the three-case explanation.
- **Documentation review status:** see `grammar-rules-human-review.md`. **Flagged high-risk** — see that document's risk notes.

## 3. `ARTICLE_ZERO_GENERAL`

- **CEFR:** `A2`
- **titleRu:** Отсутствие артикля в общем значении · **titleEn:** Zero article for general reference
- **shortExplanationRu:** Перед существительными во множественном числе и перед неисчисляемыми существительными в общем значении артикль не нужен.
- **explanationRu:** Когда вы говорите о предмете **в общем смысле**, а не о конкретном экземпляре, артикль не ставится — ни a/an, ни the. Это касается двух разных случаев: (1) множественное число в общем значении — например, «Regulators check compliance reports» имеет в виду регуляторов вообще, любых, а не какую-то конкретную группу; (2) неисчисляемые существительные в общем значении — «Compliance requires attention» имеет в виду комплаенс как понятие, а не какой-то конкретный случай. **Отличие от `ARTICLE_THE_SPECIFIC`:** если то же самое множественное число или неисчисляемое существительное уточнено и стало конкретным для собеседника, the возвращается — сравните «Regulators check reports» (регуляторы вообще) и «The regulators who visited us yesterday checked our reports» (конкретные регуляторы, уточнённые придаточным).
- **pattern/formula:** `∅ + plural noun` или `∅ + uncountable noun` (общее значение, не конкретный случай)
- **whenToUse:** обобщения; определения; утверждения общего характера.
- **whenNotToUse:** когда существительное уточнено/конкретно (тогда the); когда это единственный экземпляр в единственном числе (тогда a/an или the).
- **signalWords/structural signals:** «in general», «usually», обобщающие подлежащие в начале предложения.
- **prerequisites:** `COUNTABLE_UNCOUNTABLE`, `ARTICLE_THE_SPECIFIC` (для контраста).
- **typical mistakes (RU-спикеры):** (1) добавление the к общим утверждениям («The compliance is important» вместо «Compliance is important»); (2) добавление a/an к множественному числу («a regulators»); (3) обратная ошибка — убирание the там, где предмет уже конкретен.
- **incorrect/correct pairs:**
  1. `The compliance is important for every company.` → `Compliance is important for every company.` (context: утверждение общего характера, ни о какой конкретной компании речи нет)
  2. `We need a evidences to proceed.` → `We need evidence to proceed.` (context: evidence в общем смысле, не конкретный набор улик)
  3. `The regulators usually check documents twice a year.` → `Regulators usually check documents twice a year.` (context: «usually» сигнализирует обобщение — регуляторы вообще, не конкретная группа, которая уже упоминалась)
- **everyday example:** `Dogs need regular exercise.` (context: собаки вообще, не чьи-то конкретные)
- **professional/compliance example:** `Regulators expect full transparency from financial institutions.` (context: регуляторы и финансовые организации в общем смысле, не конкретные)
- **contrast example:** `Regulators check reports.` (context: в общем) vs `The regulators who visited us yesterday checked our reports.` (context: конкретные, уточнённые придаточным)
- **edge cases:** обобщение через единственное число с the тоже возможно в формальном стиле («The whale is a mammal») — не входит в MVP-объяснение, чтобы не перегружать A2.
- **resolver hints:** see `grammar-resolver-test-cases.md` row `ARTICLE_ZERO_GENERAL`.
- **related ErrorType:** `ARTICLE` · **related MicroCategory:** `ARTICLES`
- **exercise templates:**
  1. `fill_blank`: «___ compliance is a shared responsibility.» (no article) → `""` (пустой пропуск)
  2. `choice`: «We need ___ evidence before we act.» [a / the / (no article)] → `(no article)`
  3. `correct_sentence`: «The regulators expect the transparency from banks.» → «Regulators expect transparency from banks.»
- **sourceRefs:** British Council LearnEnglish — "Articles: 'the' or no article" and the general-things note from "Articles: 'a', 'an', 'the'"; **plus, this round, a second independent source** — Oxford Learner's Dictionaries / OUP ELT (`grammar-source-verification.md` #3) — no legacy repo table covers zero-article at all, this content is newly authored against these sources. The earlier single-source gap flagged in `grammar-rules-human-review.md` is addressed by this second citation; both remain `PARTIALLY_VERIFIED` (snippet-based), not a direct page read — see `grammar-source-verification.md` for why.
- **Documentation review status:** see `grammar-rules-human-review.md`. **Flagged high-risk.**
...
- **shortExplanationRu:** Finished-time выражения (yesterday, last year, in 2024) обычно требуют Past Simple. Present Perfect (have/has + V3) — когда важен результат сейчас или момент времени не указан.
- **explanationRu:** Оба времени говорят о прошлом, но с разным фокусом. **Present Perfect** (have/has + причастие прошедшего времени) используется, когда: (1) точное время действия не указано и не важно — важен результат сейчас («I have finished the report» — отчёт готов сейчас); (2) есть связь с настоящим моментом через already/yet/ever/never/just («Have you ever reviewed this policy?»); (3) действие в периоде, который ещё не закончился, часто с since/for. **Past Simple** используется, когда указано **конкретное завершённое время**: yesterday, last year, in 2024, two days ago, when I was at university. **Уточнение (эта формулировка исправлена по итогам ревью):** signal words — это не безошибочный автоматический классификатор, а полезный, но не абсолютный ориентир; в стандартном учебном английском finished-time выражения (yesterday, last year, in 2024) обычно требуют Past Simple, и это правило достаточно надёжно для повседневной практики, но итоговое решение всегда зависит от контекста — важен ли для говорящего именно результат сейчас или именно момент в прошлом. Since/for требуют отдельного анализа: разница не в самом слове, а в том, продолжается ли ситуация сейчас («I have worked here since 2020» — работаю до сих пор) или уже закончилась («I worked there for three years» — уже не работаю).
- **pattern/formula:** Present Perfect: `have/has + V3` (result now / unspecified time) · Past Simple: `V-ed/irregular` (finished time: yesterday, last year, in 2024)
- **whenToUse (Present Perfect):** результат важен сейчас; время не указано; already/yet/ever/never/just; since/for для незакончившегося периода.
- **whenNotToUse (Present Perfect):** как правило, не сочетается с finished-time выражениями (yesterday, last year, in 2024, two days ago, when + прошедшее время) — тогда обычно нужен Past Simple.
- **signalWords/structural signals:** Present Perfect → already, yet, ever, never, just, since, for, so far (надёжные, но не абсолютные ориентиры); Past Simple → yesterday, last week/year/month, in [конкретный год], ago, when I was….
- **prerequisites:** `PAST_SIMPLE_FORM`.
- **typical mistakes (RU-спикеры):** (1) Present Perfect с finished-time markers («I have seen this policy yesterday» — прямая калька с русского прошедшего вида); (2) Past Simple там, где нужен результат-Present Perfect без указания времени; (3) путаница since (точка отсчёта) и for (длительность).
- **incorrect/correct pairs:**
...
- **contrast example:** `I have finished it yesterday.` (нестандартное сочетание в учебном английском) → `I finished it yesterday.`
...
- **resolver hints:** see `grammar-resolver-test-cases.md` row `PAST_SIMPLE_VS_PRESENT_PERFECT` — note this is a `HIGH`/`MEDIUM` split (direct have/has signal vs. compensating finished-time-marker heuristic), never treated as a deterministic classifier on its own.
...
- **edge cases:** arrive at (здание/точка) vs arrive in (город/страна) — оба верны для разных по масштабу мест; **arrive home/here/there не требует предлога вообще** («We arrived home yesterday», не «arrived at home») — уточнение, добавленное по итогам source verification, отсутствовало в исходном черновике.
...
- **shortExplanationRu:** Неисчисляемые существительные (information, advice, evidence, equipment) не имеют множественного числа и не берут a/an. В обычной речи для количества чаще используется a lot of, а much — в вопросах, отрицаниях и более формальном стиле.
- **explanationRu:** Некоторые английские существительные, которые в русском кажутся «счётными», в английском грамматически **неисчисляемы**: у них нет формы множественного числа (нельзя «informations», «advices», «evidences»), и они не берут артикль a/an. Частые проблемные слова: information, advice, evidence, research, equipment, knowledge, feedback, furniture, software, news, progress. **Уточнение (эта формулировка исправлена по итогам ревью):** для количества неисчисляемых в обычных утвердительных предложениях чаще звучит естественно **a lot of** («We collected a lot of evidence» / «substantial evidence» в более формальном/деловом контексте) — **much** тоже грамматически верно, но в утвердительных предложениях воспринимается более формально/книжно; much типичен в вопросах и отрицаниях («Is there much evidence?», «There isn't much evidence»). Many/few — только для исчисляемого множественного числа («many reports», «few documents»), не для неисчисляемых. Some/any работают с обоими типами одинаково.
- **pattern/formula:** uncountable, affirmative: `a lot of / substantial + noun (no plural, no a/an)`; uncountable, question/negative/formal: `much + noun` also natural · countable plural: `many/few + noun-s`
...
- **whenNotToUse:** не добавляйте -s к неисчисляемым; не используйте a/an с неисчисляемыми в общем значении (см. также `ARTICLE_ZERO_GENERAL`).
- **signalWords/structural signals:** much/many, little/few, a lot of, some/any — сигнал проверить исчисляемость следующего существительного.
- **prerequisites:** полезно связать с `ARTICLE_ZERO_GENERAL`.
- **typical mistakes (RU-спикеры):** (1) множественное число у неисчисляемых («informations», «evidences», «advices»); (2) many/few вместо much/a lot of («many information»); (3) a/an перед неисчисляемым («an information»).
- **incorrect/correct pairs:**
...
- **edge cases:** некоторые слова могут быть и исчисляемыми, и неисчисляемыми в разных значениях (work — неисчисляемо в значении «работа вообще», но «a work of art» — исчисляемо в значении «произведение»).
...
- **Documentation review status:** see `grammar-rules-human-review.md`. **Flagged high-risk** (per the brief, despite good legacy coverage — corrected in this round per source verification).
