...
| Разговор с ИИ | 18 сценариев в 9 режимах (повседневные + комплаенс/работа), живой транскрипт, подсказки на русском, разбор после диалога, сохранение фраз, честная статистика |
...
│   └── src/modules/
│       ├── ai/            слой ИИ: 12 ролей-промптов, LLM-клиент, дев-фолбэки
...
Отдельные системные промпты для 12 ролей (раздел 17 PRD): оценщик диагностики,
оценщик переводов, оценщик ответов в повторении, корректор грамматики,
генератор уроков, разговорный партнёр, оценщик разговора, классификатор
ошибок, экстрактор фраз, упрощатель текстов, генератор плана, генератор
адаптивных микро-уроков. Все ответы — строгий JSON с серверной валидацией.
...
| Speaking — text conversation engine | `DONE` | 18 сценариев/9 режимов, живой транскрипт, разбор после диалога, сохранение фраз |
...
| Production validation всех AI use cases/ролей | `PARTIAL` | 12 ролей-промптов реализованы и покрыты dev-режимом тестов, но систематической проверки всех 12 ролей на реальных production-ответах (не дев-фолбэке) не проводилось |
...
| --- | --- | --- |
| Phrase seeds | `content/seed-phrases.ts` — 31 фраза, без структурированных полей (collocations, sense, active/passive mastery, source/version) | Нужно 600–1000 элементов с полной схемой (§C), MVP-итерация — 100–150 (§C) |
...
| AI prompts | 12 системных промптов-ролей (слой ИИ, раздел 17 PRD) — сами по себе рабочие, но грамматика/примеры каждый раз изобретаются ИИ заново | AI должен получать проверенный контент из базы, не изобретать (см. «AI role & constraints» ниже) |
...
- Подробная документация Phase 2A (аудит, доменная модель, retrieval-архитектура,
  editorial workflow, migration plan, MVP-срезы, метрики, risk register,
  architecture decisions) — см. [`docs/content-pedagogy/`](./docs/content-pedagogy/README.md).
  Документация зафиксирована; реализация (миграция, сиды, код) ещё не начата.
...
- Критерии приёмки: существующие 31 фраза и связанный прогресс
