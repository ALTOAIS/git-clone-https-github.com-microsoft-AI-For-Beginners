...
describe('classifyMicroCategory', () => {
  it('распознаёт третье лицо единственного числа', () => {
    expect(
      classifyMicroCategory(
        'He work in the office.',
        'He works in the office.',
      ),
    ).toBe('THIRD_PERSON_SINGULAR');
  });

  it('распознаёт пропущенный/добавленный артикль', () => {
    expect(classifyMicroCategory('I am officer.', 'I am an officer.')).toBe(
      'ARTICLES',
    );
  });
  ...
  it('распознаёт неверный предлог', () => {
    expect(
      classifyMicroCategory(
        'She is responsible of this project.',
  ...
  it('распознаёт неверный порядок слов', () => {
    expect(
      classifyMicroCategory(
        'Always I check the report.',
        'I always check the report.',
      ),
    ).toBe('WORD_ORDER');
  });
  ...
  it('распознаёт comply/compliance', () => {
    expect(
      classifyMicroCategory(
        'The company must compliance with the law.',
  ...
  it('распознаёт комплаенс-лексику', () => {
    expect(
      classifyMicroCategory(
        'We found a breach of policy.',
        'We found a violation of policy.',
      ),
